...
const LocationMap = ({ posts, setSelectedWard, selectedWard }) => {
  ...
  useEffect(() => {
    // Calculate the center for each ward's GeoJSON feature
    const centers = {};
    ghmcWards.features.forEach(feature => {
      const wardName = feature.properties.ward_name;
      if (wardName) {
        // A simple way to find a center: average the coordinates of the bounding box
  ...
  const getWardEmotion = (wardName) => {
    ...
    if (wardPosts.length === 0) return 'Neutral'; // Default color

    // Simple logic: return emotion of the first post
